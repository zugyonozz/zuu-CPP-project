...
template <size_t N>
using index_type = std::conditional_t<N <= 255, uint8_t, std::conditional_t<N <= 65535, uint16_t, uint32_t>> ;
...
requires (sizeof...(Ts) > 0)
class generic {
    static_assert(detail::all_trivial_v<Ts...>, "All types must be trivially copyable for optimal performance") ;
